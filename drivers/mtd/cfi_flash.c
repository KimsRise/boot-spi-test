...
flash_info_t *flash_get_info(ulong base)
{
	...
	// shlee2302 130506
	printf("CONFIG_SYS_MAX_FLASH_BANKS:%d\n", CONFIG_SYS_MAX_FLASH_BANKS);
	printf("size:%d start:%x base:%x\n"
		, flash_info[0].size, flash_info[0].start[0], base);
	...
	return i == CONFIG_SYS_MAX_FLASH_BANKS ? &flash_info[0] : info;
}
#endif
...
#define BANK_BASE(i)	(((phys_addr_t [CFI_MAX_FLASH_BANKS])CONFIG_SYS_FLASH_BANKS_LIST)[i])
	...
	for (i = 0; i < CONFIG_SYS_MAX_FLASH_BANKS; ++i) {
		...
		if (flash_info[i].flash_id == FLASH_UNKNOWN) {
			...
			// shlee2302 [[
			flash_info[i].size = (8*1024*1024);			/* total bank size in bytes		*/
			flash_info[i].sector_count = 2048;		/* number of erase units		*/
			flash_info[i].flash_id = FLASH_MAN_CFI;		/* combined device & manufacturer code	*/

			for(aa=0; aa<CONFIG_SYS_MAX_FLASH_SECT; aa++){
				flash_info[i].start[aa] = aa*2048;   /* virtual sector start address */
				flash_info[i].protect[aa] = 0; /* sector protection status	*/
			}
#ifdef CONFIG_SYS_FLASH_CFI
			flash_info[i].portwidth = CONFIG_SYS_FLASH_CFI_WIDTH;		/* the width of the port		*/
			flash_info[i].chipwidth = FLASH_CFI_BY8;		/* the width of the chip		*/
			flash_info[i].buffer_size = 1;		/* # of bytes in write buffer		*/
			flash_info[i].erase_blk_tout = (1<<4)*(1<<1);		/* maximum block erase timeout		*/
			flash_info[i].write_tout = ((1 << 3)*(1 << 1) + 999) / 1000;		/* maximum write timeout		*/
			flash_info[i].buffer_write_tout = 1;	/* maximum buffer write timeout		*/
			flash_info[i].vendor = 2;			/* the primary vendor id		*/
			flash_info[i].cmd_reset = AMD_CMD_RESET;		/* vendor specific reset command	*/
			flash_info[i].interface = 0x0001;		/* used for x8/x16 adjustments		*/
			flash_info[i].legacy_unlock = 0;		/* support Intel legacy (un)locking	*/
			flash_info[i].manufacturer_id = 0xBF;	/* manufacturer id			*/
			flash_info[i].device_id = 0x236D;		/* device id				*/
			flash_info[i].device_id2 = 0;		/* extended device id			*/
			flash_info[i].ext_addr = 0;		/* extended query table address		*/
			flash_info[i].cfi_version = 0;		/* cfi version				*/
			flash_info[i].cfi_offset = 0x555;		/* offset for cfi query			*/
			flash_info[i].addr_unlock1 = 0x555;		/* unlock address 1 for AMD flash roms  */
			flash_info[i].addr_unlock2 = 0x2AA;		/* unlock address 2 for AMD flash roms  */
			flash_info[i].name = "CFI conformant";		/* human-readable name	                */
#endif
